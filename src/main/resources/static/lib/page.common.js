...
;(function(){
    ...
    window.euiFn = {
        ...
        /**把js数组datagrid数据行转为json数组:euiFn.paramsArray(datagrids,fields);fields为指定字段,字段间用,分开*/
        paramsArray : function(datagrids,fields){
            if(datagrids == null || datagrids.length <= 0 || datagrids == undefined)return null;
            if(fields == null || fields == ''){
                var paramsArray = '[';
                for(var key in datagrids){
                    var jsonArray = datagrids[key];
                    var json = '{';
                    for(var k in jsonArray){
                        var value = jsonArray[k];
                        json = json + '"'+k+'":"'+value+'",';
                    }
                    json = json.substring(0,(json.length-1)) + '}';
                    paramsArray = paramsArray + json+',';
                }
                return paramsArray.substring(0,(paramsArray.length-1)) +']';
            } else {
                var paramsArray = '[';
                for(var key in datagrids){
                    var jsonArray = datagrids[key];
    ...
    window.layerFn = {
        ...
        center : function(code,msg){
            ...
            var exitIndex = top.layer.alert('<div style="font-size: 12px;"><img src='+imagerUrl+' style="vertical-align:middle;margin-right:3px;"/>'+msg+'</div>',{title : AppKey.title,area : AppKey.layerArea});
            layerFn.EscLayer(exitIndex);
        },
