...
<div id='${nameSpace}_edit_dialog' style="display:none;">
    <table cellspacing="4" cellpadding="0" width="100%" class="view" border="0" style="margin-top:6px;">
        ...
        <#else>
		<tr>
            <td style="text-align:right;padding-right:4px;">${ld.column_comment !'字段注释'}</td>
...
<script type="text/javascript">
    ;(function ($){
        ...
        thisPage = {
                ...
                $(_self.datagrid).datagrid({
                    ...
                    columns : [[
						<#list listData as ld>
                            <#if "${keyId}" == "${ld.column_name}">
                            <#else>
						{field:'${ld.column_name}',title:'${ld.column_comment !'字段注释'}',width:40,align:'left'},
                            </#if>
                        </#list>
            ...
            /**搜索*/
            search : function(){
                euiFn.refreshDatagrid(thisPage.datagrid,winFn.formAjaxParams('#search_${nameSpace}'),1);
            },
            /**type=1编辑;2添加;其他未知;*/
