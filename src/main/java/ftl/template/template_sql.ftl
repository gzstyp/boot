...
<mapper namespace="${nameSpace}">
    ...
    <update id="updateEditBatch" parameterType="HashMap" useGeneratedKeys="true" keyProperty="${keyId}">
        INSERT INTO ${tableName} (
        <trim suffixOverrides=",">
        <#list listData as ld>
        <#if ld_has_next>
        ...
        <#list listEdit as le>
            <#if le_has_next>
        <if test="${le.column_name} != null and ${le.column_name} != ''">${le.column_name} = VALUES (${le.column_name}),</if>
            <#else>
        <if test="${le.column_name} != null and ${le.column_name} != ''">${le.column_name} = VALUES (${le.column_name})</if>
            </#if>
        </#list>
