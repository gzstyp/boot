...
public final class ToolString implements Serializable {
    ...
    */
    public final static HashMap<String,String> parseJsonObjectOriginal(final Object json){
        ...
        try {
            final JSONObject jsonObject = JSONObject.parseObject(json.toString());
            for (String key : jsonObject.keySet()){
                jsonMap.put(key,(jsonObject.get(key) == null || jsonObject.get(key).toString().length() <= 0) ? "" : String.valueOf(jsonObject.get(key)));
            }
            return jsonMap;
        } catch (Exception e){
            return jsonMap;
    ...
    */
    ...
    /**
     * <strong style='color:#f69;'>解析json数组字符串,ArrayList里的HashMap的key全是大写</strong>
     * @提示 <strong style='color:#369;'>json对象就是以{"开头,即HashMap<String,String>;json数组就是以[{"开头,即ArrayList<HashMap<String,String>>;json全都是String</strong>
     * @作者 田应平
     * @创建时间 2018年8月3日 09:22:45
     * @QQ号码 444141300
     * @主页 http://www.fwtai.com
    */
    public final static ArrayList<HashMap<String,String>> parseJsonArrayUpper(final Object jsonArray){
        final ArrayList<HashMap<String, String>> listResult = new ArrayList<HashMap<String, String>>();
        if(isBlank(jsonArray)) return listResult;
        try {
            ArrayList<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();//初始化,以免出现空指针异常
            ...
            if(list != null && list.size() > 0){
                for (int i = 0; i < list.size(); i++){
                    final HashMap<String, String> hashMap = new HashMap<String, String>();
                    for (String key : list.get(i).keySet()){
                        hashMap.put(key.toUpperCase(Locale.CHINESE),(list.get(i).get(key) == null || list.get(i).get(key).toString().length() <= 0) ? "" : list.get(i).get(key).toString());
                    }
                    listResult.add(hashMap);
                }
                return listResult;
        ...
        try {
            ...
            if(list != null && list.size() > 0){
                for (int i = 0; i < list.size(); i++){
                    final HashMap<String, String> hashMap = new HashMap<String, String>();
                    for (String key : list.get(i).keySet()){
                        hashMap.put(key,(list.get(i).get(key) == null || list.get(i).get(key).toString().length() <= 0) ? "" : list.get(i).get(key).toString());
                    }
                    listResult.add(hashMap);
                }
                return listResult;
    ...
    */
    ...
    /**
     * 解析json数组字符串,ArrayList里的HashMap<String,String>的key未做处理
     * @param
     * @作者 田应平
     * @QQ 444141300
     * @创建时间 2018年12月3日 09:21:02
    */
    public final static ArrayList<HashMap<String,String>> parseListHashMap(final Object array){
        ...
        for(int i = 0; i < jsonArray.size();i++){
            ...
            for(final String key : json.keySet()){
                map.put(key,setBlank(json.get(key),null));
            }
            list.add(map);
        }
        return list;
